...
class AutoPropertyAssembler extends ManualPropertyAssembler {
    ...
    public function assemble($component) {
        ...
        $container = $this->componentDef->getContainer();
        ...
        foreach ($propDescs as $propDesc) {
            if ($this->componentDef->hasPropertyDef($propDesc->getPropertyName())) {
                continue;
            }
            $value   = null;
            try {
                ...
                \seasar\log\S2Logger::getInstance(__CLASS__)->info("no component found for typehint property. [{$this->componentDef->getComponentClass()->getName()}::\${$propDesc->getPropertyName()}]", __METHOD__);
                continue;
            } catch (\seasar\container\exception\TooManyRegistrationRuntimeException $e) {
                \seasar\log\S2Logger::getInstance(__CLASS__)->info("too many component found for typehint property. ignored. [{$this->componentDef->getComponentClass()->getName()}::\${$propDesc->getPropertyName()}]", __METHOD__);
                continue;
            }
