<?php
class PHPUnit2AllTestTask extends Task {
    ...
    public function main(){
        ...
        $suite = new PHPUnit2_Framework_TestSuite($this->name);
        foreach($includefiles as $files){
            $c = count($files);
            for($i = 0; $i < $c; $i++){
                require_once($files[$i]['absolute']);
                $this->log('add : ' . $files[$i]['name']);
                $suite->addTest(new PHPUnit2_Framework_TestSuite(new ReflectionClass($files[$i]['class'])));
            }
        }
