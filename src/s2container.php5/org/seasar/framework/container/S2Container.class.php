 ...
 * <ul>
 * <li>{@link S2Container_ConstructorAssembler コンストラクタ・インジェクション}<br>
 *     コンストラクタ引数を利用してコンポーネントをセットします。 </li>
 * <li>{@link S2Container_PropertyAssembler セッター・インジェクション}<br>
 *     セッターメソッドを利用してコンポーネントをセットします。 </li>
 * <li>{@link S2Container_MethodAssembler メソッド・インジェクション}<br>
...
interface S2Container extends S2Container_MetaDefAware
{
    ...
    /**
     * <b>outerComponent</b>のクラスをキーとして登録された
     * {@link S2Container_ComponentDef コンポーネント定義}に従って、必要なコンポーネントのインジェクションを実行します。
     * アスペクト、コンストラクタ・インジェクションは適用できません。
     * 
     * <p>
     * {@link S2Container_ComponentDef コンポーネント定義}のインスタンス定義は
     * outerでなくてはなりません。
     * </p>
     * 
     * @param object $outerComponent 外部コンポーネント
     * @param string $componentName コンポーネント定義のキー (名前)
     * @throws S2Container_ClassUnmatchRuntimeException 適合するコンポーネント定義が見つからない場合
     */
     ...
     * </p>
     * 
     * @param object|string|S2Container_ComponentDef $component コンポーネント
     ...
     * @throws S2Container_ContainerNotRegisteredRuntimeException S2コンテナが見つからない場合
     */    
