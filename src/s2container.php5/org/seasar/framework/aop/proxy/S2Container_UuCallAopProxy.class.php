...
class S2Container_UuCallAopProxy {
     ...
     * 
     */
    function __construct($targetClass,$map,$args,$params)
    {
        $this->targetClass_ = $targetClass;
        $this->methodInterceptorsMap_ = $map;
        $this->parameters_ = $params;

        if (!$this->targetClass_->isInterface() and
            !$this->targetClass_->isAbstract()) {
            $this->target_ = 
                S2Container_ConstructorUtil::newInstance($this->targetClass_,
                                                         $args);
        }        
    }
        ...
        if (array_key_exists($name,$this->methodInterceptorsMap_)) {
            ...
            if (!is_object($this->target_)) {
                if ($this->targetClass_->isInterface()) {
                    $msg = "target class [" .
                        $this->targetClass_->getName() . "] is interface. ";
                } else {
                    $msg = "target class [" . 
                        $this->targetClass_->getName() . "] ";
                }

                throw new S2Container_S2RuntimeException('ESSR0043',
                array($name,$msg));
            }
