...
set DEFAULT_PHING_HOME=%~dp0\etc\phing\
...
if "%PHING_PHP_OPTION%" == "" goto set_php_option
goto run
goto cleanup

:run
%PHP_COMMAND% %PHING_PHP_OPTION% -d html_errors=off -qC %PHING_HOME%\bin\phing.php %1 %2 %3 %4 %5 %6 %7 %8 %9
goto cleanup
...
:set_php_option
echo WARNING: we will turn off output_buffering!
echo if you do not want to turn off it, please define PHING_PHP_OPTION with no string
set PHING_PHP_OPTION=-d output_buffering=off -d output_handler= -d max_execution_time=
goto init
